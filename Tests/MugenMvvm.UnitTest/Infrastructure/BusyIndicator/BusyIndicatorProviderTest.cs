...
{
    public class BusyIndicatorProviderTest : UnitTestBase
    {
        #region Methods
        ...
        {
            var busyIndicator = GetBusyIndicator();
            var token = busyIndicator.Begin(busyIndicator);
            busyIndicator.BusyInfo.ShouldNotBeNull();
        ...
        [Fact]
        public void BeginWithMessageShouldNotifyListeners()
        {
            var message1 = "1";
            var message2 = "2";
            ...
            busyIndicator.Begin(message1);
            list.Count.ShouldEqual(1);
            list[0].Token.Message.ShouldEqual(message1);

            list.Clear();
            busyIndicator.Begin(message2);
            list.Count.ShouldEqual(1);
            list[0].Token.Message.ShouldEqual(message2);
        }

        [Fact]
        public void BeginWithTokenShouldNotifyListeners()
        {
            var rootIndicator = GetBusyIndicator();
            ...
            list.Count.ShouldEqual(1);
            list[0].Token.Message.ShouldEqual(busyToken1.Message);

            list.Clear();
            busyIndicator.Begin(busyToken2);
            list.Count.ShouldEqual(1);
            list[0].Token.Message.ShouldEqual(busyToken2.Message);
        }

        [Fact]
        public void ProviderShouldNotifyListenersOnBusyChanged()
        {
        ...
        [Fact]
        public void ProviderShouldNotifyListenersOnBusyChangedParallel()
        {
            const int count = 1000;
            var notificationCount = 0;
            var listener = new TestBusyIndicatorProviderListener {OnBusyChanged = () => Interlocked.Increment(ref notificationCount)};
        ...
        public void BeginWithTokenShouldSuspendTokenParallel1()
        {
            ...
            foreach (var indicatorProvider in providers)
                tasks.Add(Task.Run(() => indicatorProvider.Suspend()));
