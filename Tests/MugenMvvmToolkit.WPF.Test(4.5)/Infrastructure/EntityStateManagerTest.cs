...
namespace MugenMvvmToolkit.Test.Infrastructure
{
    ...
    public class EntityStateManagerTest : TestBase
    {
        ...
        public void ProviderShouldTrackPropertyChanges()
        {
            ...
            snapshot.HasChanges(stateModel, ToolkitExtensions.GetPropertyName<EntityStateModel>(model => model.Guid)).ShouldBeFalse();
            snapshot.HasChanges(stateModel, ToolkitExtensions.GetPropertyName<EntityStateModel>(model => model.String)).ShouldBeFalse();
            snapshot.HasChanges(stateModel, ToolkitExtensions.GetPropertyName<EntityStateModel>(model => model.Int)).ShouldBeFalse();

            stateModel.Int = int.MaxValue;
            stateModel.String = null;
            stateModel.Guid = Guid.Empty;

            snapshot.HasChanges(stateModel, ToolkitExtensions.GetPropertyName<EntityStateModel>(model => model.Guid)).ShouldBeTrue();
            snapshot.HasChanges(stateModel, ToolkitExtensions.GetPropertyName<EntityStateModel>(model => model.String)).ShouldBeTrue();
            snapshot.HasChanges(stateModel, ToolkitExtensions.GetPropertyName<EntityStateModel>(model => model.Int)).ShouldBeTrue();
