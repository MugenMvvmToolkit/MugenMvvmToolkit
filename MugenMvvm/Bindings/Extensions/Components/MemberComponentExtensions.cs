...
namespace MugenMvvm.Bindings.Extensions.Components
{
    public static class MemberComponentExtensions
    {
            ...
            EnumFlags<MemberFlags> flags, object request, IReadOnlyMetadataContext? metadata)
        {
            ...
            {
                var members = c.TryGetMembers(memberManager, type, memberTypes, flags, request, metadata);
                if (!members.IsEmpty)
                    return members;
            }
            ...
            IReadOnlyMetadataContext? metadata)
        {
            ...
            foreach (var c in components)
                result.AddRange(c.TryGetMembers(memberManager, type, name, memberTypes, metadata));
