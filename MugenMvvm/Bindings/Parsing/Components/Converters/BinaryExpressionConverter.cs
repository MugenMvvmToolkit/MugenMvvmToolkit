...
namespace MugenMvvm.Bindings.Parsing.Components.Converters
{
    public sealed class BinaryExpressionConverter : IExpressionConverterComponent<Expression>, IHasPriority
    {
        ...
        {
            if (expression is TypeBinaryExpression {NodeType: ExpressionType.TypeIs} typeIsExp)
                return new BinaryExpressionNode(BinaryTokenType.TypeIs, context.Convert(typeIsExp.Expression), ConstantExpressionNode.Get(typeIsExp.TypeOperand));
            if (expression is UnaryExpression {NodeType: ExpressionType.TypeAs} typeAsExp)
                return new BinaryExpressionNode(BinaryTokenType.TypeAs, context.Convert(typeAsExp.Operand), ConstantExpressionNode.Get(typeAsExp.Type));
