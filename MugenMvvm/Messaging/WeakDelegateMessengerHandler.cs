...
namespace MugenMvvm.Messaging
{
        ...
        where TTarget : class
    {
        ...
        public WeakDelegateMessengerHandler(TTarget target, Action<TTarget, object?, TMessage, IMessageContext> action, IWeakReferenceManager? weakReferenceManager = null)
        {
            Should.NotBeNull(target, nameof(target));
            Should.NotBeNull(action, nameof(action));
            _reference = target.ToWeakReference(weakReferenceManager);
            _action = action;
        }

        public WeakDelegateMessengerHandler(Action<object?, TMessage, IMessageContext> action, IWeakReferenceManager? weakReferenceManager = null,
            IReflectionManager? reflectionManager = null)
        {
            Should.NotBeNull(action, nameof(action));
            ...
            _reference = action.Target.ToWeakReference(weakReferenceManager);
            _action = action.GetMethodInfo()!.GetMethodInvoker<Action<TTarget, object, TMessage, IMessageContext>>(reflectionManager)!;
        }
