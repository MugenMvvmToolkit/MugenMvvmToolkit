...
namespace MugenMvvm.UnitTest.Binding.Members.Builders
{
    public class PropertyBuilderTest : UnitTestBase
    {
        ...
        [InlineData(true)]
        [InlineData(false)]
        public void StaticAutoGeneratedShouldRaiseOnChange(bool observable)
        {
            var message = "m";
        ...
        [Fact]
        public void WrapShouldUseWrappedMember()
        {
            var value = new object();
            var newValue = new object();

            var wrappedMember = new PropertyBuilder<object, object?>("auto", typeof(object), typeof(object)).Build();
            int invokeCount = 0;
            TestComponentSubscriber.Subscribe(new TestMemberManagerComponent
            {
                ...
                {
                    ++invokeCount;
                    type.ShouldEqual(this.GetType());
                    memberType.ShouldEqual(MemberType.Accessor);
                    arg3.ShouldEqual(MemberFlags.InstanceAll);
            ...
            var member = new PropertyBuilder<object, object>("wrap", typeof(object), typeof(object)).WrapMember("auto").Build();
            member.GetValue(this, DefaultMetadata).ShouldBeNull();
            invokeCount.ShouldEqual(1);
            member.TryObserve(this, listener);

            listener.InvokeCount.ShouldEqual(0);
            wrappedMember.SetValue(this, value, DefaultMetadata);
            listener.InvokeCount.ShouldEqual(1);
            member.GetValue(this, DefaultMetadata).ShouldEqual(value);

            wrappedMember.SetValue(this, null, DefaultMetadata);
            listener.InvokeCount.ShouldEqual(2);

            member.SetValue(this, newValue, DefaultMetadata);
