...
namespace MugenMvvm.Binding.Core
{
    public sealed class ModeBindingComponentProvider : BindingComponentProviderComponent.IProvider, IExpressionVisitor
    {
        ...
        {
            if (context.ComponentBuilders.ContainsKey(BindingParameterNameConstant.EventHandler) && !(sourceExpression is IBindingMemberExpressionNode))
            {
                sourceExpression.Accept(this, metadata);
                context.ComponentBuilders[BindingParameterNameConstant.Mode] = InstanceBindingComponentBuilder.NoneMode;
                return;
            }
            ...
            foreach (var builder in _bindingModes)
            {
                ...
                {
                    context.ComponentBuilders[BindingParameterNameConstant.Mode] = builder.Value;
                    return;
                }
            }
        }
